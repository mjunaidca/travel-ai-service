...
from vertexai.preview.generative_models import (Content,
                                                ...
                                                Part)
import vertexai
PROJECT_ID = os.environ.get('travel-ai-408413')  # Your Google Cloud Project ID
LOCATION = os.environ.get('761051996953')  # Your Google Cloud Project Region
...
with tab3:
    ...
    image_undst, screens_undst, diagrams_undst, recommendations, sim_diff = st.tabs(
        ["Furniture recommendation", "Oven instructions", "ER diagrams", "Glasses recommendation", "Math reasoning"])

    with image_undst:
        st.markdown("""In this demo, you will be presented with a scene (e.g., a living room) and will use the Gemini model to perform visual understanding. You will see how Gemini can be used to recommend an item (e.g., a chair) from a list of furniture options as input. You can use Gemini to recommend a chair that would complement the given scene and will be provided with its rationale for such selections from the provided list.
                    """)
        ...
        room_image_urls = "https://storage.googleapis.com/" + \
            room_image_uri.split("gs://")[1]
        chair_1_image_urls = "https://storage.googleapis.com/" + \
            chair_1_image_uri.split("gs://")[1]
        chair_2_image_urls = "https://storage.googleapis.com/" + \
            chair_2_image_uri.split("gs://")[1]
        chair_3_image_urls = "https://storage.googleapis.com/" + \
            chair_3_image_uri.split("gs://")[1]
        chair_4_image_urls = "https://storage.googleapis.com/" + \
            chair_4_image_uri.split("gs://")[1]

        room_image = Part.from_uri(room_image_uri, mime_type="image/jpeg")
        chair_1_image = Part.from_uri(
            chair_1_image_uri, mime_type="image/jpeg")
        chair_2_image = Part.from_uri(
            chair_2_image_uri, mime_type="image/jpeg")
        chair_3_image = Part.from_uri(
            chair_3_image_uri, mime_type="image/jpeg")
        chair_4_image = Part.from_uri(
            chair_4_image_uri, mime_type="image/jpeg")

        st.image(room_image_urls, width=350, caption="Image of a living room")
        st.image([chair_1_image_urls, chair_2_image_urls, chair_3_image_urls, chair_4_image_urls],
                 width=200, caption=["Chair 1", "Chair 2", "Chair 3", "Chair 4"])
    ...
    with screens_undst:
        stove_screen_uri = "gs://github-repo/img/gemini/multimodality_usecases_overview/stove.jpg"
        stove_screen_url = "https://storage.googleapis.com/" + \
            stove_screen_uri.split("gs://")[1]

        st.write("Equipped with the ability to extract information from visual elements on screens, Gemini can analyze screenshots, icons, and layouts to provide a holistic understanding of the depicted scene.")
        # cooking_what = st.radio("What are you cooking?",["Turkey","Pizza","Cake","Bread"],key="cooking_what",horizontal=True)
        stove_screen_img = Part.from_uri(
            stove_screen_uri, mime_type="image/jpeg")
...
"""
    ...
    with diagrams_undst:
        ...
        er_diag_url = "https://storage.googleapis.com/" + \
            er_diag_uri.split("gs://")[1]
        ...
        prompt = """Document the entities and relationships in this ER diagram.
                """
        tab1, tab2 = st.tabs(["Response", "Prompt"])
        er_diag_img_description = st.button(
            "Generate!", key="er_diag_img_description")
        with tab1:
    ...
    with recommendations:
        ...
        compare_img_1_url = "https://storage.googleapis.com/" + \
            compare_img_1_uri.split("gs://")[1]
        compare_img_2_url = "https://storage.googleapis.com/" + \
            compare_img_2_uri.split("gs://")[1]

        st.write("""Gemini is capable of image comparison and providing recommendations. This may be useful in industries like e-commerce and retail.
                    Below is an example of choosing which pair of glasses would be better suited to various face types:""")
        compare_img_1_img = Part.from_uri(
            compare_img_1_uri, mime_type="image/jpeg")
        compare_img_2_img = Part.from_uri(
            compare_img_2_uri, mime_type="image/jpeg")
        face_type = st.radio("What is your face shape?", [
                             "Oval", "Round", "Square", "Heart", "Diamond"], key="face_type", horizontal=True)
        output_type = st.radio("Select the output type", [
                               "text", "table", "json"], key="output_type", horizontal=True)
        st.image([compare_img_1_url, compare_img_2_url], width=350,
                 caption=["Glasses type 1", "Glasses type 2"])
        st.write(f"Our expectation: Suggest which glasses type is better for the {
    ...
    with sim_diff:
        ...
        math_image_url = "https://storage.googleapis.com/" + \
            math_image_uri.split("gs://")[1]
        st.write("Gemini can also recognize math formulas and equations and extract specific information from them. This capability is particularly useful for generating explanations for math problems, as shown below.")
...
with tab4:
    ...
    with vide_desc:
        ...
        vide_desc_uri = "https://www.youtube.com/watch?v=EdqNft_S2Tg"
        video_desc_url = "https://storage.googleapis.com/" + \
            vide_desc_uri.split("gs://")[1]
        if vide_desc_uri:
    ...
    with video_tags:
        ...
        video_tags_uri = "https://photos.onedrive.com/share/D0A1CC6B3C26223B!129364?cid=D0A1CC6B3C26223B&resId=D0A1CC6B3C26223B!129364&authkey=!ANzcybRHlx5CqDs&ithint=video&e=3SzPhn"
        video_tags_url = "https://storage.googleapis.com/" + \
            video_tags_uri.split("gs://")[1]
        if video_tags_url:
            video_tags_img = Part.from_uri(
                video_tags_uri, mime_type="video/mp4")
    ...
    with video_highlights:
        st.markdown("""Below is another example of using Gemini to ask questions about objects, people or the context, as shown in the video about Pixel 8 below:""")
        video_highlights_uri = "gs://github-repo/img/gemini/multimodality_usecases_overview/pixel8.mp4"
        video_highlights_url = "https://storage.googleapis.com/" + \
...
Provide the answer in table format. 
    ...
    with video_geoloaction:
        ...
        video_geoloaction_uri = "gs://github-repo/img/gemini/multimodality_usecases_overview/bus.mp4"
        video_geoloaction_url = "https://storage.googleapis.com/" + \
